...
const UI = {
  ...
  showLeaderboard() {
    ...
    S.lbType = 'chips';
    UI.switchLeaderboard('chips');
    if (S.myName) S.socket.emit('get_stats', { name: S.myName });
  },
  closeLeaderboard() { $('leaderboardOverlay').classList.remove('show'); },
  switchLeaderboard(type) {
    S.lbType = type;
    document.querySelectorAll('.lb-tab').forEach(t => t.classList.toggle('active', t.dataset.type === type));
    $('leaderboardList').innerHTML = '<div style="text-align:center;color:var(--muted);padding:20px">Yüklənir…</div>';
    S.socket.emit('get_leaderboard', { type });
  },
  renderLeaderboard(data) {
    const list = $('leaderboardList');
    list.innerHTML = '';
    if (!data.entries || !data.entries.length) {
      list.innerHTML = '<div style="text-align:center;color:var(--muted);padding:20px">Hələ heç bir oyunçu yoxdur</div>';
      return;
    }
    data.entries.forEach((p, idx) => {
      const row = document.createElement('div');
      const isMe = p.name === S.myName;
      row.className = 'lb-row' + (isMe ? ' is-me' : '');
      const rankCls = idx === 0 ? 'gold' : idx === 1 ? 'silver' : idx === 2 ? 'bronze' : '';
      const rankSym = idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : '#' + (idx + 1);
      const winRate = p.hands_played > 0 ? Math.round((p.hands_won / p.hands_played) * 100) : 0;
      const mainVal = data.type === 'winnings'
        ? fmt(p.total_winnings) + ' qalibiyyət'
        : '💰 ' + fmt(p.chips);
      const subInfo = data.type === 'winnings'
        ? `${p.hands_won}/${p.hands_played} əl · ${winRate}% qalibiyyət`
        : `${p.hands_played} əl oynanıb · ${winRate}% qalibiyyət`;
      row.innerHTML = `
        <span class="lb-rank ${rankCls}">${rankSym}</span>
        <span class="lb-av">${esc(p.avatar || '🦁')}</span>
        <div style="flex:1;min-width:0">
          <div class="lb-name">${esc(p.name)}</div>
          <span class="lb-sub">${subInfo}</span>
        </div>
        <span class="lb-val">${mainVal}</span>`;
      list.appendChild(row);
    });
  },
  renderMyStats(stats) {
    if (!stats) { $('myStatsBox').innerHTML = ''; return; }
    const winRate = stats.hands_played > 0
      ? Math.round((stats.hands_won / stats.hands_played) * 100) : 0;
    $('myStatsBox').innerHTML = `
      <div class="my-stats-title">📊 Sənin Statistikan</div>
      <div class="my-stats-grid">
        <div class="ms-cell"><span class="ms-lbl">Chips:</span><span class="ms-val">${fmt(stats.chips)}</span></div>
        <div class="ms-cell"><span class="ms-lbl">Ümumi qalib:</span><span class="ms-val">${fmt(stats.total_winnings)}</span></div>
        <div class="ms-cell"><span class="ms-lbl">Oynanmış əl:</span><span class="ms-val">${stats.hands_played}</span></div>
        <div class="ms-cell"><span class="ms-lbl">Qalibiyyət:</span><span class="ms-val">${winRate}%</span></div>
        <div class="ms-cell" style="grid-column:span 2"><span class="ms-lbl">Ən böyük pot:</span><span class="ms-val">${fmt(stats.biggest_pot || 0)}</span></div>
      </div>`;
  },
...
window.addEventListener('load', () => {
  ...
  S.socket.on('your_profile', d => {
    S.myChips = d.chips;
    S.myStats = d;
    $('hdrCh').textContent = '💰 ' + fmt(d.chips);
  });
  S.socket.on('leaderboard_data', d => UI.renderLeaderboard(d));
  S.socket.on('stats_data', d => UI.renderMyStats(d));
});
